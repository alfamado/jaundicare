...
// import { SafeAreaView } from "react-native-safe-area-context";
// import { Ionicons } from "@expo/vector-icons";
// import { useHistory } from "../../hooks/useHistory"; // Importing your new hook
// import { Colors, Fonts, Radius, Shadow } from "../../constants/colors";
// import { decisionConfig } from "../../constants/decisionMap";

// export default function HistoryScreen() {
//   // Pulling the cleaned data from your custom hook
//   const { history: items, hasHistory } = useHistory();

//   // If you still want a refresh control, you can trigger a sync or just leave it as a visual indicator
//   const [refreshing, setRefreshing] = React.useState(false);
//   const onRefresh = React.useCallback(async () => {
//     setRefreshing(true);
//     // Add logic here if you want to force a background sync of the store
...
//         ) : (
//           items.map((item, index) => {
//             // Mapping your triage_level string to your existing decision config UI
//             const dm = decisionConfig(item.triage_level);
//             const isFirst = index === 0;
...
//             return (
//               <View key={item.id} style={[s.card, isFirst && s.cardLatest]}>
//                 <View style={s.cardTop}>
//                   <View style={[s.badge, { backgroundColor: dm.color + "18", borderColor: dm.color }]}>
//                     <Text style={s.badgeIcon}>{dm.icon}</Text>
//                     <Text style={[s.badgeText, { color: dm.color }]}>{dm.label}</Text>
...
//                 {/* Displaying the baby name mapped from your profile inside the hook */}
//                 <View style={s.metaRow}>
//                   <Ionicons name="person-outline" size={12} color={Colors.brownLight} />
//                   <Text style={s.metaText}>{item.baby_name}</Text>
//                 </View>

//                 <Text style={s.reason} numberOfLines={2}>{item.decision_reason}</Text>
//               </View>
//             );
//           })
//         )}
...
// // Structured type map to comfortably handle combined local-offline and api states
...
// export default function HistoryScreen() {
//   // 1. Fetch live history data via network cache query layer
//   const { data: onlineHistory, isRefetching, refetch } = useQuery({
//     queryKey: ["history"],
//     queryFn:  screeningApi.history,
//   });

//   // 2. Fetch locally stored state history (handles offline-created items)
//   const { history: localHistory } = useHistory();

//   // 3. Unify online items and local store history seamlessly
//   const apiItems = (onlineHistory ?? []) as any[];

//   const unifiedItems = React.useMemo(() => {
//     // Cast online records safely to our unified layout template
//     const combined: UnifiedHistoryItem[] = apiItems.map((item) => ({
//       screening_id: item.screening_id,
//       final_decision: item.final_decision || item.triage_level || "GREEN",
//       final_decision_reason: item.final_decision_reason || item.decision_reason,
//       created_at: item.created_at,
//       baby_age_hours: item.baby_age_hours,
//       image_confidence: item.image_confidence,
//       image_prediction: item.image_prediction,
//       isOffline: false,
//     }));

//     // Inject matching pending records collected when user was offline
//     localHistory.forEach((localItem) => {
//       const exists = combined.some(
//         (item) => item.screening_id === localItem.id || item.id === localItem.id
//       );
//       if (!exists) {
//         combined.push({
//           screening_id: localItem.id,
//           id: localItem.id,
//           final_decision: localItem.triage_level,
//           final_decision_reason: localItem.decision_reason,
...
//     // Order chronological layout: newest events at the top
...
//             return (
//               <View key={item.screening_id || item.id || index} style={[s.card, isFirst && s.cardLatest]}>
//                 <View style={s.cardTop}>
//                   <View style={[s.badge, { backgroundColor: dm.color + "18", borderColor: dm.color }]}>
//                     <Text style={s.badgeIcon}>{dm.icon}</Text>
//                     <Text style={[s.badgeText, { color: dm.color }]}>{dm.label}</Text>
...
//     gap: 4
//   },
//   offlineTagText: { fontFamily: Fonts.medium, fontSize: 11, color: Colors.brown },
//   latestTag: {
//     backgroundColor: Colors.coral,
//     paddingHorizontal: 8,
//     paddingVertical:   3,
...
/**
 * JaundiCare — Comprehensive Screening History Pipeline
 * Seamlessly stitches network screening payloads together with locally cached
 * offline triage actions. Ensures robust fallback UI layout structures.
 */
...
export default function HistoryScreen() {
  ...
  // Stabilize base collection logic by passing the raw nullable response straight to the dependency array
  const unifiedItems = React.useMemo(() => {
    const apiRecords = onlineHistory ?? [];
    
    const combined: UnifiedHistoryItem[] = apiRecords.map((item: any) => ({
      screening_id: item.screening_id,
      final_decision: item.final_decision || item.triage_level || "GREEN",
      final_decision_reason: item.final_decision_reason || item.decision_reason,
      created_at: item.created_at,
      ...
      >
        ...
        {unifiedItems.length === 0 ? (
          ...
          unifiedItems.map((item, index) => {
            ...
            // Secure explicit composite string array maps
            const componentKey = `history_${item.screening_id || item.id || index}`;

            return (
              <View key={componentKey} style={[s.card, isFirst && s.cardLatest]}>
                <View style={s.cardTop}>
                  <View style={[s.badge, { backgroundColor: dm.color + "18", borderColor: dm.color }]}>
                    <Text style={s.badgeIcon}>{dm.icon}</Text>
                    <Text style={[s.badgeText, { color: dm.color }]}>{dm.label}</Text>
