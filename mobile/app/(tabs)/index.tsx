...
/**
 * JaundiCare — Parent/Worker Core Landing Dashboard
 * Maps live medical screening status markers, triggers refresh pipelines, 
 * handles age calculations, and references warning protocols for user safety.
 */
...
export default function DashboardScreen() {
  ...
  // Sync server data to client cache store on fetch changes safely
  useEffect(() => {
    if (profile) setProfile(profile);
  }, [profile, setProfile]);
  ...
  return (
    <SafeAreaView style={s.safe}>
      ...
      >
        ...
        {/* Quick action screening switch button */}
        <TouchableOpacity
          style={s.primaryBtn}
