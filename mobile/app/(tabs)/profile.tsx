...
/**
 * JaundiCare — Profile Setup Screen
 * Captures core demographic information, checks gestational age parameters, 
 * validates strict date/time configurations, and configures localized background reminders.
 */
...
export default function ProfileScreen() {
  ...
  // Track initialization status to prevent mid-typing overwrite loops
  const [isInitialized, setIsInitialized] = useState(false);

  useEffect(() => {
    if (profile?.exists && !isInitialized) {
      setForm({
        baby_name:             profile.baby_name             ?? "",
        parent_name:           profile.parent_name           ?? "",
        date_of_birth:         profile.date_of_birth         ?? "",
  ...
  const { mutate: saveProfile, isPending } = useMutation({
    mutationFn: () => profileApi.save({
      ...
      sex:                   form.sex || undefined,
      gestational_age_weeks: form.gestational_age_weeks
        ? parseInt(form.gestational_age_weeks, 10) : undefined,
    }),
    onSuccess: async (saved) => {
      setProfile(saved);
      await queryClient.invalidateQueries({ queryKey: ["profile"] });
  ...
  const handleSave = () => {
    ...
    if (!timeRegex.test(form.time_of_birth.trim())) {
      return Alert.alert("Invalid Format", "Time of birth must use the 24-hour HH:MM layout.");
    }
            ...
            {[
              ...
              val ? (
                <View key={lbl as string} style={s.savedRow}>
                  <Text style={s.savedLabel}>{lbl}</Text>
                  <Text style={s.savedValue}>{val}</Text>
                </View>
              ) : null
            )}
          </View>
