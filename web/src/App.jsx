...
function AssistantHub({ t, language }) {
  ...
  const labels = { "newborn-care": "BabyAI", "immunisation-ng": "VaccineAI" };
  ...
  return <aside className={`chat-widget ${open ? "chat-open" : ""}`}><button className="chat-toggle" onClick={() => setOpen(!open)}>{open ? t("web.chat.close", "Close assistants") : t("web.chat.open", "Ask an assistant")}</button>{open && <div className="chat-body"><div className="assistant-tabs"><button className={assistant === "newborn-care" ? "assistant-active" : ""} onClick={() => setAssistant("newborn-care")}>BabyAI</button><button className={assistant === "immunisation-ng" ? "assistant-active" : ""} onClick={() => setAssistant("immunisation-ng")}>VaccineAI</button></div><p><strong>{labels[assistant]}</strong><br />{assistant === "newborn-care" ? "Pregnancy, newborn and early-childhood support." : "Pregnancy and child vaccination guidance."}</p><div className="suggestion-row">{suggestions.map((suggestion) => <button disabled={busy} key={suggestion} onClick={() => void sendQuestion(suggestion)}>{suggestion}</button>)}</div><div className="chat-messages" aria-live="polite">{chats[assistant].length === 0 && <span>{t("web.chat.empty", "Choose a suggested question or type your own below.")}</span>}{chats[assistant].map((item, index) => <p className={item.role} key={`${item.role}-${index}`}>{item.text}</p>)}{busy && <span>{t("web.chat.thinking", "Thinking…")}</span>}</div><form onSubmit={(event) => { event.preventDefault(); void sendQuestion(message); }}><input value={message} onChange={(event) => setMessage(event.target.value)} placeholder={t("web.chat.placeholder", "Type your question")} disabled={busy} /><button aria-label="Send question" disabled={busy}>↑</button></form><small>{t("trust.title", "This is a screening support tool.")}</small></div>}</aside>;
}
